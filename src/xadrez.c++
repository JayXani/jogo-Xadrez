...
int SDL_interface(){
    if(SDL_Init(SDL_INIT_EVERYTHING) >=0){
        cout<<"\n Inicializando o SDL...";
        window = SDL_CreateWindow("J O G O _ X A D R E Z ",
            50,50, WIDTH, HEIGHT, SDL_WINDOW_SHOWN);

        if(window != NULL){
            cout<<"\n Janela Inicializada com sucesso \n";
            render = SDL_CreateRenderer(window, -3, 0);

            if(render != NULL){
            }else{
                cout<<"Erro ao criar o render..\n";
            }
        }else{
            cout<<"Erro ao criar a janela..\n";
        }
    }else{
        cout<<"Erro ao iniciar o SDL..\n";
    }
}
